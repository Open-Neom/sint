...
class SintDelegate extends RouterDelegate<RouteDecoder>
  ...
  final Iterable<SintPage> Function(RouteDecoder currentNavStack)?
      pickPagesForRootNavigator;

  List<RouteDecoder> get activePages => _activePages;

  final _routeTree = RouteParser(routes: []);

  List<SintPage> get registeredRoutes => _routeTree.routes;

  void addPages(List<SintPage> getPages) {
  ...
  void addPage(SintPage getPage) {
    _routeTree.addRoute(getPage);
  }

  void removePage(SintPage getPage) {
  ...
  Future<RouteDecoder?> runMiddleware(RouteDecoder config) async {
    ...
    for (var item in middlewares) {
      ...
      if (config != redirectRes) {
        config.route?.completer?.complete();
        Sint.log('Redirect to ${redirectRes.pageSettings?.name}');
      }
  ...
  ///
  /// visual pages must have [SintPage.participatesInRootNavigator] set to true
  Iterable<SintPage> getVisualPages(RouteDecoder? currentHistory) {
    final res = currentHistory!.currentTreeBranch
        .where((r) => r.participatesInRootNavigator != null);
    if (res.isEmpty) {
      //default behavior, all router participate in root navigator
  ...
  Future<T?> to<T>(
    ...
    final getPage = SintPage<T>(
      name: routeName,
      opaque: opaque ?? true,
      page: page,
      gestureWidth: gestureWidth,
      showCupertinoParallax: showCupertinoParallax,
      popGesture: popGesture ?? Sint.defaultPopGesture,
      transition: transition ?? Sint.defaultTransition,
      curve: curve ?? Sint.defaultTransitionCurve,
      fullscreenDialog: fullscreenDialog,
      bindings: bindings,
      transitionDuration: duration ?? Sint.defaultTransitionDuration,
      preventDuplicateHandlingMode: preventDuplicateHandlingMode,
    );
  ...
  /// Removes router according to [PopMode]
  ...
  Future<T?> _replace<T>(PageSettings arguments, SintPage<T> page) async {
  ...
  Future<T?> _push<T>(RouteDecoder decoder, {bool rebuildStack = true}) async {
    ...
    /// There are no duplicate router in the stack
    if (onStackPage == null) {
      _activePages.add(res);
    } else {
      /// There are duplicate router, reorder
      switch (preventDuplicateHandlingMode) {
        case PreventDuplicateHandlingMode.doNothing:
          break;
        case PreventDuplicateHandlingMode.reorderRoutes:
